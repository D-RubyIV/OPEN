...
import random
import queue
from services import checkBalance
import json
...
class WorkerThread(QThread):
    ...
    def __init__(self, account, main_window_object, thread_id ,service):
        super().__init__()
    ...
    def handle_account(self, account, proxy_list,client_key,service):
        ...
        while True:
            ...
            if service == "Capsolver":
                solver_result = captcha.createTask("Capsolver",client_key, captcha_image)['solution']['text']
            else:
                taskID = captcha.createTask("2Captcha", client_key, captcha_image)['taskId']
                print("Task ID:", taskID)

                while True:  
                    solver = captcha.getTaskResult("2Captcha", client_key, taskID)
                    if solver['status'] == 'processing':
                        print("Solver status is 'processing'. Waiting for 5 seconds...")
                        time.sleep(5)
                        continue

                    else:
                        solver_result=solver['solution']['text']
                        print("Solver status:", solver['solution']['text'])
                        break  
    ...
    def runcode(self, account,service): 

        file_name = 'account.txt'
        proxy_file = self.read_proxy_list('proxy.txt')
        client_key = read_token.read_token_from_ini('token.ini')
        #handle_account(self, account, proxy_list,client_key,service)
        self.handle_account(account,proxy_file,client_key,service)
   

    def run(self):
        self.runcode(self.account,self.service)
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        MainWindow.setFixedSize(1108, 703) 
        ...
        """)
        self.pushButton_check_balance = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_check_balance.setGeometry(QtCore.QRect(280, 120, 91, 28))  
        self.pushButton_check_balance.setObjectName("pushButton_check_balance")
         

        self.pushButton_check_balance.setStyleSheet("""
            QPushButton {
        ...
        self.threads = []
        self.pushButton_check_balance.clicked.connect(self.checkBalance)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton_check_balance.setText("Check Balance") 
    ...
    def start_threads(self):
        ...
        for account in accounts:
            number += 1
            self.MYTHREAD[number] = WorkerThread(account, self ,number,self.comboBox_3.currentText())
    ...
    def checkBalance(self):
        ...
        print(client_key)
        try:
            balance = checkBalance.get_balance(self.comboBox_3.currentText(),client_key).json()['balance']
            QMessageBox.information(None, "Balance "+self.comboBox_3.currentText(), f"Balance : {balance} .", QMessageBox.Ok)
        except:
                QMessageBox.warning(None, "Warning", "Key is invaild", QMessageBox.Ok)
