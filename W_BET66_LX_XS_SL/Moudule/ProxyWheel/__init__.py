    ...
    def getNewIp(self, api_key):
        ...
        if '|' in api_key:
            ...
            while True:
                ...
                listProvine = [i["id"] for i in requests.get("https://wwproxy.com/api/client/province?search_text=", timeout=10).json()["data"]]
                ip = requests.get(f"https://wwproxy.com/api/client/proxy/available?key={api_key}&provinceId={random.choice(listProvine)}", timeout=10).json()
                print(ip)
                if ip["errorCode"] == 0:
                    ipcc = ip['data']['proxy']
