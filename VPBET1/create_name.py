...
for name in names:
    ...
    pass_rut = ''.join(random.choice(string.digits) for _ in range(random.randint(6,10)))
    ...
    if my_name == "":
            with open('username.txt', 'a', encoding='utf-8') as f:
                f.write(f'{username}|{password}|{ten_nguoi}|{phone}|{stk}|{pass_rut}\n')
    else:
        ten_nguoi = my_name.upper()
        with open('username.txt', 'a', encoding='utf-8') as f:
            # f.write(f'{username}|{password}|{ten_nguoi}|{ngay_sinh}|{city}|{stk}|{bank}|{pass_rut}\n')
            f.write(f'{username}|{password}|{ten_nguoi}|{stk}|{pass_rut}\n')
