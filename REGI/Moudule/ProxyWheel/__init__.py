...
class ProxyWheel:
    ...
    def getNewIp(self, api_key):
        ...
        ip_me = requests.get(
            "https://api.ipify.org/?format=json", headers=headers
        ).json()["ip"]
        if "|" in api_key:
            ...
            while True:
                try:
                    ip = requests.get(
                        f"https://app.proxydt.com/api/public/proxy/get-new-proxy?license={api_key}&authen_ips={ip_me}",
                        headers=headers,
                    ).json()
                    print(ip)
                ...
                if ip["code"] == 1:
                    ...
                    if (
                        "Không tìm thấy license" in str(ip)
                        or "Yêu cầu license key & whitelist Ip!" in str(ip)
                        or "Token không hợp lệ." in str(ip)
                    ):
                        print(ip)
                        return "KEYEXPIRED"
            ...
            while True:
                ...
                try:
                    ...
                    if ip["success"]:
                        ...
                        timee = int(REGREX.findall(r'\d+', ip["description"])[0])  # type: ignore
                        for ll in range(timee, -1, -1):
                            print(
                                f"Vui lòng đợi {str(ll)} để đổi ip lần nữa        ",
                                end="\r",
